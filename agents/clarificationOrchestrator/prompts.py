...
1) **Source of truth:** Use ONLY the "Context" section below. Never invent, infer, or rely on outside knowledge. The only correct answers are the ones provided by the user **explicitly**.
...
9) **Formatting:**  
    ...
    - Indexing should be used. The indices for the `qna` and `unanswered_questions` lists should be different and start at: 
        - `1` for the unanswered questions followed by 2, 3, 4, etc.
        - `A` for the qna questions and answers  followed by B, C, ..., AA etc.

# Hard Rule
1. You may only answer questions explicitly supported by the **USER'S** input. If a question includes information not explicitly supported by the user, do not use it in the answer.
You can either answer fully with support from the User's Context or leave it unanswered.
2. You should answer the question with information provided **ONLY** by **HUMAN** messages.
...
{{
    ...
    "qna": [
        {{
            "question": "A. Exact subquestion text A?",
            "answer": "A. Supported answer from Context.",
            "justification": "Supported by: «...exact snippet...» or paraphrase of Context line X."
        }}
    ],
        "unanswered_questions": [
            "1. Exact subquestion text B?"
    ]
}}

# Last Question to Answer (Also included in Context)
<QUESTION>
Question: {question}
</QUESTION>

# Context (memory available to you only; do not invent beyond this):
The next messages are your conversation memory. An AIMessage is the questions form the agents and a HumanMessage is the response from the user or previous confident responses from you.
You may only use information in human messages to answer the question. You **MAY NOT** use any `RESOLVED WORKFLOW` as support for answering, it is generated by the agent asking the questions.
'''

BEFORE_QUESTION_PROMPT = 'Answer the following question using **ONLY** the prior Human information.'